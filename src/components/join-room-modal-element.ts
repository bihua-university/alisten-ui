          ...
          <div class="flex gap-3">
            <div class="relative flex-1">
              <input type="text" .value=${this.searchKeyword} placeholder="搜索房间名称..."
                class="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3 pl-10 text-sm text-white placeholder-white/40 focus:outline-none focus:border-purple-500/50 focus:bg-white/10 transition-all"
                @input=${(e: InputEvent) => {
            ...
            <button class="bg-purple-600 hover:bg-purple-500 active:scale-95 text-white rounded-2xl px-4 py-3 transition-all shadow-lg shadow-purple-600/20 flex items-center gap-2 whitespace-nowrap text-sm font-medium"
              @click=${() => {
          ...
          <div class="max-h-72 overflow-y-auto space-y-2">
            ${this.isSearching
              ...
              <div class="text-center py-8 text-white/40">
                <div class="w-8 h-8 border-2 border-purple-500/30 border-t-purple-500 rounded-full animate-spin mx-auto mb-2"></div>
            ...
            `
              ...
              <div class="group p-3 rounded-2xl bg-white/[0.03] hover:bg-white/[0.06] transition-all cursor-pointer border border-transparent
                ${this.selectedRoomId === room.id ? 'bg-purple-600/10 border-purple-500/40' : ''}"
                ...
                <div class="flex items-center justify-between">
                  ...
                  <div class="ml-3 flex-shrink-0 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all
                    ${this.selectedRoomId === room.id ? 'bg-purple-600 border-purple-600' : 'border-white/20'}">
      ...
      <alisten-modal title="确认加入房间" headerIcon="fa-solid fa-music" size="sm"
        ...
        <div class="rounded-2xl p-4 mb-4 bg-white/[0.03]">
          <div class="flex items-center mb-3">
            <div class="w-10 h-10 rounded-xl bg-purple-600/20 flex items-center justify-center mr-3">
              ${unsafeSVG(icons.logIn(20, 'text-purple-400'))}
            </div>
            <div class="flex-1 min-w-0">
        ...
        ${room.needPwd
          ...
          <div class="mb-4">
            <input type="password" .value=${this.confirmPassword} placeholder="请输入房间密码"
              class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-white/30 focus:outline-none focus:border-purple-500/50 transition-all"
        ...
        <div class="flex gap-3">
          ...
          <button class="flex-1 bg-purple-600 hover:bg-purple-500 active:scale-95 text-white rounded-xl py-3 transition-all shadow-lg shadow-purple-600/20 font-medium
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <textarea .value=${this.createDesc} placeholder="请输入房间描述（可选）"
              class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-white/30 focus:outline-none focus:border-purple-500/50 transition-all resize-none"
          ...
          <div>
            <div class="flex justify-between items-center mb-2">
              ...
              <button type="button" class="text-xs text-purple-400 hover:text-purple-300 transition-colors"
                @click=${() => {
