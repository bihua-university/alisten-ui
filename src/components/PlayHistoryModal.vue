<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center">
    ...
    <div
      class="relative bg-dark border border-white/20 rounded-xl w-full max-w-4xl h-[90vh] flex flex-col overflow-hidden m-4"
    >
      <!-- 头部 -->
        ...
        <div class="flex items-center space-x-2">
          ...
          <div class="relative">
            <input
              ...
              type="text"
              placeholder="搜索歌曲..."
              class="bg-white/10 text-white placeholder-gray-400 rounded-lg px-3 py-2 text-sm w-48 focus:outline-none focus:ring-2 focus:ring-purple-500"
            >
          ...
          <!-- 功能按钮 -->
          <button
            class="bg-purple-500/20 hover:bg-purple-500/30 text-purple-400 rounded-lg px-3 py-2 text-sm transition-all"
            @click="showStats = !showStats"
          >
            <i class="fa-solid fa-chart-bar mr-1" />
            统计
          </button>

          <button
            class="bg-green-500/20 hover:bg-green-500/30 text-green-400 rounded-lg px-3 py-2 text-sm transition-all"
            @click="exportHistory"
          >
          ...
          <button
            class="bg-red-500/20 hover:bg-red-500/30 text-red-400 rounded-lg px-3 py-2 text-sm transition-all"
          ...
          <button
            class="text-gray-400 hover:text-white transition-colors rounded-lg p-2"
            @click="$emit('close')"
          >
      ...
      <div class="flex-1 overflow-hidden">
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-6">
          ...
          <div v-for="group in filteredHistoryByDate" :key="group.date" class="space-y-2">
            <h3 class="text-sm font-medium text-gray-300 sticky top-0 bg-dark/80 backdrop-blur py-2">
              {{ group.date }}
            </h3>
            <div class="space-y-1">
              <div
                ...
                >
                  <template #actions="{ music }">
                    ...
                    <div class="flex items-center space-x-1">
                      <button
                        class="bg-purple-500/20 hover:bg-purple-500/30 text-purple-400 rounded-full w-8 h-8 flex items-center justify-center transition-all"
                        @click="pickMusic(music, music.source || 'netease')"
                      >
                        <i class="fa-solid fa-plus text-xs" />
                      </button>
                      <button
                        class="bg-red-500/20 hover:bg-red-500/30 text-red-400 rounded-full w-8 h-8 flex items-center justify-center transition-all"
    ...
    <div v-if="showClearConfirm" class="fixed inset-0 z-60 flex items-center justify-center">
      ...
      <div class="relative bg-dark border border-white/20 rounded-xl p-6 max-w-md mx-4">
        ...
        <div class="flex justify-end space-x-3">
          <button
            class="bg-gray-500/20 hover:bg-gray-500/30 text-gray-400 rounded-lg px-4 py-2 transition-all"
            @click="showClearConfirm = false"
          >
            取消
          </button>
          <button
            class="bg-red-500/20 hover:bg-red-500/30 text-red-400 rounded-lg px-4 py-2 transition-all"
