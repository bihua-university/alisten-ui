...
export class SettingsModalElement extends LitElement {
  ...
  disconnectedCallback() {
    ...
    userSettingsStore.removeEventListener('change', this.handleUserSettingsChange)
    performanceStore.removeEventListener('change', this.handlePerformanceChange)
    playerStore.removeEventListener('change', this.handlePlayerChange)

    const nameInput = this.querySelector('input[type="text"]') as HTMLInputElement | null
    const emailInput = this.querySelector('input[type="email"]') as HTMLInputElement | null
  ...
  render() {
    return html`
      <alisten-modal title="设置" headerIcon="fa-solid fa-cog" .open=${true} @close=${this.handleClose}>
        <div class="space-y-6">
          <!-- Profile -->
          <div>
            <h4 class="text-sm font-bold text-white mb-3 flex items-center gap-2">
              ${unsafeSVG(icons.users(16, 'text-purple-400'))}
              个人资料
            </h4>
            <div class="space-y-3">
              <!-- Avatar Preview -->
              <div class="flex items-center gap-4 p-3 bg-white/5 rounded-xl">
                <img src=${this.avatarUrl} alt="头像预览" class="w-14 h-14 rounded-full bg-white/10 object-cover">
                <div class="flex-1 min-w-0">
                  <p class="text-sm font-medium text-white truncate">${this.userName || '匿名用户'}</p>
                  <p class="text-xs text-white/40 truncate">${this.userEmail || '未设置邮箱'}</p>
                </div>
              </div>
              <div>
              ...
              <div>
                <label class="text-xs text-white/50 mb-1 block">邮箱 (用于 Gravatar 头像)</label>
                <input type="email" .value=${this.userEmail}
                  class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-white/30 focus:outline-none focus:border-purple-500/50 transition-all"
                  placeholder="输入邮箱"
                  @input=${(e: InputEvent) => this.userEmail = (e.target as HTMLInputElement).value}>
                ${!userSettingsStore.emailValidation.valid ? html`<p class="text-xs text-red-400 mt-1">${userSettingsStore.emailValidation.message}</p>` : nothing}
              </div>
            </div>
          </div>
          ...
          <div>
            <h4 class="text-sm font-bold text-white mb-3 flex items-center gap-2">
              ${unsafeSVG(icons.volume2(16, 'text-indigo-400'))}
              音量控制
            </h4>
            <div class="flex items-center gap-3">
              <button class="p-2 rounded-lg bg-white/5 hover:bg-white/10 transition-colors shrink-0"
                @click=${() => playerStore.toggleMute()}>
                ${unsafeSVG(this.isMuted ? icons.volumeX(18, 'text-white/50') : icons.volume2(18, 'text-white/70'))}
              </button>
              <div class="flex-1 h-2 bg-white/10 rounded-full cursor-pointer relative" @click=${this.handleVolumeClick}>
                <div class="absolute top-0 left-0 h-full bg-gradient-to-r from-purple-500 to-indigo-500 rounded-full transition-all volume-bar" style="width: ${this.isMuted ? 0 : this.volume}%"></div>
              </div>
              <span class="text-xs text-white/50 w-10 text-right shrink-0">${this.isMuted ? 0 : Math.round(this.volume)}%</span>
            </div>
          </div>
          ...
          <div>
            <h4 class="text-sm font-bold text-white mb-3 flex items-center gap-2">
              ${unsafeSVG(icons.listMusic(16, 'text-purple-400'))}
              播放模式
            </h4>
            <div class="flex flex-col gap-2 mb-3">
              ${([
                ...
                return html`
                  <button class="text-left p-3 rounded-xl transition-all border ${active ? 'bg-purple-600/20 border-purple-500/40' : 'bg-white/5 border-transparent hover:bg-white/10'}"
                    @click=${() => userSettingsStore.setPlayMode(mode)}>
                    <div class="flex items-center justify-between">
                      <div>
                        <div class="text-sm font-medium ${active ? 'text-white' : 'text-white/70'} flex items-center">
                          ${unsafeSVG(icon)}
                          ${title}
                        </div>
                        <p class="text-xs ${active ? 'text-white/70' : 'text-white/40'} leading-relaxed pl-6">${desc}</p>
                      </div>
                      <div class="w-4 h-4 rounded-full border-2 flex items-center justify-center transition-colors shrink-0 ml-2 ${active ? 'border-purple-400' : 'border-white/30'}">
                        ${active ? html`<div class="w-2 h-2 rounded-full bg-purple-400"></div>` : nothing}
                      </div>
                    </div>
                  </button>
                `
          ...
          <div>
            <h4 class="text-sm font-bold text-white mb-3 flex items-center gap-2">
              ${unsafeSVG(icons.zap(16, 'text-amber-400'))}
              性能设置
            </h4>
            <div class="flex flex-col gap-2 mb-3">
              ${([
                ...
                return html`
                  <button class="text-left p-3 rounded-xl transition-all border ${active ? 'bg-purple-600/20 border-purple-500/40' : 'bg-white/5 border-transparent hover:bg-white/10'}"
                    ...
                    <div class="flex items-center gap-2 mb-1">
                      <div class="w-2 h-2 rounded-full ${active ? 'bg-purple-400' : 'bg-white/20'}"></div>
                      <span class="text-sm font-medium ${active ? 'text-white' : 'text-white/70'}">${title}</span>
                    </div>
                    <p class="text-xs ${active ? 'text-white/70' : 'text-white/40'} leading-relaxed pl-4">${desc}</p>
                  </button>
                `
              })}
            </div>
