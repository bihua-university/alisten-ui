        ...
        <div class="space-y-4">
          ...
          <div class="flex gap-4">
            <div class="flex-1 glass rounded-2xl p-4 text-center">
              <div class="text-2xl font-bold text-purple-400">${this.stats.totalPlays}</div>
              <div class="text-xs text-white/50 mt-1">总播放次数</div>
            </div>
            <div class="flex-1 glass rounded-2xl p-4 text-center">
              <div class="text-2xl font-bold text-indigo-400">${formatTime(this.stats.totalDuration)}</div>
          ...
          <div class="space-y-4 max-h-[50vh] overflow-y-auto">
            ...
            `
              ...
              <div>
                <h4 class="text-xs font-bold text-white/40 uppercase tracking-wider mb-2 sticky top-0 bg-[#1a1a1f] py-1">${group.date}</h4>
                <div class="space-y-1">
                  ${group.items.map(item => html`
                    <div class="flex items-center gap-3 p-2 rounded-xl hover:bg-white/5 transition-colors group">
                      ...
                      <div class="flex-1 min-w-0">
                        ...
                        <div class="text-xs text-white/40 truncate">${item.song.artist}</div>
                      </div>
                      <span class="text-xs text-white/30 mr-1">${formatTime(item.song.duration / 1000)}</span>
          ...
          <div class="flex gap-2 pt-2 border-t border-white/5">
            <button class="flex-1 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-sm text-white/60 transition-colors flex items-center justify-center gap-2"
              @click=${() => historyStore.exportPlayHistory()}>
              ${unsafeSVG(icons.download(14))}
              导出
            </button>
            <button class="flex-1 py-2 rounded-xl bg-white/5 hover:bg-red-500/20 text-sm text-white/60 hover:text-red-400 transition-colors flex items-center justify-center gap-2"
              @click=${() => {
